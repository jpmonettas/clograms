...
```bash
clj -Sdeps '{:deps {clograms {:mvn/version "0.1.11"}}}' -m clograms.server my-clojure-project-folder clj
```

### Index a ClojureScript project :

```bash
clj -Sdeps '{:deps {clograms {:mvn/version "0.1.11"}}}' -m clograms.server my-clojurescript-project-folder cljs
```
